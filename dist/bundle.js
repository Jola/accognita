...
(() => {
  ...
  var BASE_SKILLS = [
    ...
    },
    {
      ...
      baseXpThreshold: 15,
      xpThresholdMultiplier: 1.5,
      description: "Aktiv: Ein m\xE4chtiger Nahkampfhieb mit scharfen Klauen (8 Basis-Schaden). H\xF6heres Level = st\xE4rkerer Hieb. MP-Kosten: 5.",
      mpCost: 5,
      cooldownMs: 1500,
      attackType: "melee",
      baseDamage: 8
    },
    {
      ...
      baseXpThreshold: 25,
      xpThresholdMultiplier: 1.5,
      description: "Aktiv: Fernkampf-Angriff \u2014 ein Strom lodernder Flammen \xFCber 8\xD7 K\xF6rpergr\xF6\xDFe Distanz. Verursacht direkten Feuerschaden (10 Basis). H\xF6heres Level = st\xE4rker. MP-Kosten: 8.",
      mpCost: 8,
      cooldownMs: 2500,
  ...
  var ENTITY_DEFINITIONS = [
    ...
    // ==========================================================
    {
      ...
      aggroRadius: 8,
      // 2.5 × worldSize
      skillLevels: { bite: 1, chitin_armor: 1 },
      // bite Lv1 → 7 Schaden; chitin → 10% DR
      worldSize: 3
    ...
    // ==========================================================
    {
      ...
      level: 20,
      skillDrops: [
        { skillId: "fire_breath", chance: 0.4 },
        // Seltener und wertvoller Drop
        { skillId: "claw", chance: 0.3 },
        { skillId: "bite", chance: 0.1 }
      ],
      materialDrops: [],
      hp: 500,
      speed: 35,
      // worldSize 20 × 5 = 100 → langsamer, aber massiv
      attackRangePx: 40,
      attackCooldownMs: 1800,
      attackType: "melee",
      // 2 × worldSize
      ...
      rangedAttackRangePx: 160,
      // 8 × worldSize
      rangedAttackCooldownMs: 2500,
      respawnTime: 600,
      // 10 Minuten — sehr seltener Spawn
      interactRadius: 80,
      aggroRadius: 50,
      // 2.5 × worldSize
      ...
      worldSize: 20
      // Deutlich größer als alle anderen Gegner
    },
    // ==========================================================
  ...
  function calcSkillAttackDamage(skillId, level) {
  ...
  var STAT_SCALE = 1.25;
  function canFight(def) {
    return !!(def.damage ?? def.skillLevels?.["bite"] ?? def.skillLevels?.["claw"] ?? def.rangedAttackSkillId);
  }
  ...
  function getEntityRangedDamage(def) {
  ...
  function calcEntityAi(def, instance, playerX, playerY, now) {
    ...
    const wantToRangedAttack = !!(instance.isAggro && rangedRange > 0 && rangedCooldownDone && !inMeleeRange && // Nicht schießen wenn im Nahkampf
    dist <= rangedRange);
  ...
  function tickRangedCooldown(instance, deltaMs) {
  ...
  function setRangedCooldown(instance, def) {
    instance.rangedCooldownRemaining = def.rangedAttackCooldownMs ?? 2e3;
  }
  ...
  function generateSpawns(biome, rng) {
    const spawnTable = BIOME_SPAWNS[biome] ?? [{ id: "grass", weight: 1 }];
  ...
  var GameScene = class extends Phaser.Scene {
    ...
    showFireBreath(fromX, fromY, toX, toY) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const flame = this.add.text(sx, sy, "\u{1F525}", { fontSize: "20px" }).setOrigin(0.5).setScale(1.8 / zoom).setDepth(15);
        this.tweens.add({
          targets: flame,
          x: tx,
          y: ty,
      ...
      if (result.hit) {
        ...
        if (target.currentHp <= 0) {
          ...
          if (hemolymphLv > 0) {
            ...
            addLog(`${def?.icon ?? "\u{1F41E}"} Hemolymph! ${reflectDmg} R\xFCckschlag-Schaden.`, "aggro");
          }
        }
