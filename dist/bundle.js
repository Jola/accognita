...
(() => {
  ...
  function updateNearbyPanel(entityDef, instance, state) {
    ...
    if (entityDef.damageReduction && entityDef.damageReduction > 0) {
      passiveLines.push(`\u{1F6E1}\uFE0F Chitin-Panzer: ${Math.round(entityDef.damageReduction * 100)}% DR`);
    }
    ...
    <div class="nearby-entity">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:.68em;color:var(--muted)">
          \u2694\uFE0F ${entityDef.damage ?? "\u2013"} &nbsp;|&nbsp; \u{1F4A8} ${entityDef.speed ?? "\u2013"}
        </div>
      </div>
    </div>
    ${passiveLines.length ? `<div style="font-size:.68em;color:#aaa;margin:4px 0">${passiveLines.join("<br>")}</div>` : ""}
    ${skillIcons ? `<div style="font-size:.68em;color:var(--muted);margin:2px 0">\u{1F381} ${skillIcons}</div>` : ""}
