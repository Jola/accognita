...
(() => {
  ...
  function playerAttack(player, targetInst, skillId) {
    ...
    const targetDR = targetChitinLv > 0 ? Math.min(0.1 + (targetChitinLv - 1) * 0.05, 0.7) : def.damageReduction ?? 0;
    if (targetDR > 0) {
  ...
  function entityAttack(def, instance, player) {
    ...
    const venomChance = venomLv > 0 ? 0.3 + (venomLv - 1) * 0.05 : def.venomChance ?? 0;
    if (venomChance > 0 && Math.random() < venomChance) {
      const venomDmg = venomLv > 0 ? 2 + Math.floor((venomLv - 1) * 0.5) : def.venomDamagePerTick ?? 2;
