...
// -----------------------------------------------------------
export function processTicks(target, now) {
    let hpDelta = 0;
...
// -----------------------------------------------------------
export function triggerAuras(target) {
    let reflectDmg = 0;
    for (const effect of target.statusEffects) {
        if (effect.type === "aura") {
...
// -----------------------------------------------------------
export function removeExpiredEffects(target, now) {
...
// -----------------------------------------------------------
export function clearAllEffects(target) {
...
// -----------------------------------------------------------
export function syncPassiveEffects(player) {
    const now = Date.now();
    ...
    player.statusEffects = player.statusEffects.filter((e) => {
        if (!e.sourceSkillId)
            return true; // Externe Effekte behalten
...
export function calcDamageReduction(effects) {
    let reduction = 0;
...
export function calcDamageMult(effects) {
    let mult = 1.0;
    for (const e of effects) {
        if (e.type === "stat_mod")
            mult *= e.damageMult;
...
export function calcSpeedMultiplier(effects) {
    let mult = 1.0;
    for (const e of effects) {
        if (e.type === "stat_mod")
...
// -----------------------------------------------------------
export function makeVenomEffect(venomLevel, damageOverride) {
    const now = Date.now();
