...
export class GameScene extends Phaser.Scene {
        ...
        for (const instance of this.gameState.world.entities.values()) {
            ...
            if (!instance.isAggro && instance.currentHp > 0) {
                ...
                if (instance.currentHp < scaledMax) {
                    instance.currentHp = Math.min(scaledMax, instance.currentHp + scaledMax * 0.05 * (delta / 1000));
                }
            }
        }
        // MP-Regen (1 MP/s)
    ...
    checkNearbyEntity() {
        ...
        if (nearestId !== this.lastNearbyId) {
            ...
            updateNearbyPanel(nearest ? ENTITY_MAP.get(nearest.definitionId) : undefined, nearest ?? undefined, this.gameState);
        }
    }
    // ----------------------------------------------------------
