...
function injectStyles() {
  if (document.getElementById("skillbar-styles")) return;
...
export function createSkillBar(
  ...
  const slots: (string | null)[] = new Array(NUM_SLOTS).fill(null);
  const slotEls: HTMLElement[] = [];
  const cdEls: HTMLElement[] = [];
  ...
  function update() {
    ...
    const now = Date.now();
    const allSkills: Map<string, any> = (window as any).__ALL_SKILLS;

    for (let i = 0; i < NUM_SLOTS; i++) {
      const skillId = slots[i];
      const iconEl = slotEls[i].querySelector(".sk-slot-icon") as HTMLElement;
      const cdEl = cdEls[i];
      ...
      // Cooldown-Overlay
      const expiresAt: number = player.skillCooldowns?.get(skillId) ?? 0;
  ...
  return {
    slots,
    assignSlot(index: number, skillId: string | null) {
