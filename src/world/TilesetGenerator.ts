...
export const BIOME_TILE_OFFSET: Record<string, number> = {
  forest:   0,
  swamp:    4,
  highland: 8,
  mountain: 12,
...
function draw3DEdge(
  ...
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 0, lr, lg, lb, 160);
  for (let y = 1; y < TILE_SIZE; y++) setPixel(data, 0, y, lr, lg, lb, 100);
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, TILE_SIZE - 1, dr, dg, db, 140);
  for (let y = 0; y < TILE_SIZE - 1; y++) setPixel(data, TILE_SIZE - 1, y, dr, dg, db, 100);
}
...
interface Palette { base: string; light: string; dark: string; accent: string }
...
function drawForest(data: Uint8ClampedArray, height: number, seed: number): void {
  ...
  for (let y = 0; y < TILE_SIZE; y++) {
    for (let x = 0; x < TILE_SIZE; x++) {
      ...
      setPixel(data, x, y,
        Math.min(255, Math.max(0, br + v * 35)) | 0,
        Math.min(255, Math.max(0, bg + v * 45)) | 0,
        Math.min(255, Math.max(0, bb + v * 20)) | 0,
      );
    }
  }
...
function drawSwamp(data: Uint8ClampedArray, height: number, seed: number): void {
  ...
  if (height === 0) {
    ...
    for (let y = 4; y < TILE_SIZE; y += 7) {
      for (let x = 0; x < TILE_SIZE; x++) {
        if (rng() > 0.45) setPixel(data, x, y, ar, ag, ab, 120);
      }
    }
    return;
  }
  ...
  for (let y = 0; y < TILE_SIZE; y++) {
    for (let x = 0; x < TILE_SIZE; x++) {
      ...
      setPixel(data, x, y,
        Math.min(255, Math.max(0, br + v * 25)) | 0,
        Math.min(255, Math.max(0, bg + v * 35)) | 0,
        Math.min(255, Math.max(0, bb + v * 18)) | 0,
      );
    }
  }
...
function drawHighland(data: Uint8ClampedArray, height: number, seed: number): void {
  ...
  for (let y = 0; y < TILE_SIZE; y++) {
    ...
    for (let x = 0; x < TILE_SIZE; x++) {
      ...
      setPixel(data, x, y,
        Math.min(255, Math.max(0, br + (v - streak) * 40)) | 0,
        Math.min(255, Math.max(0, bg + (v - streak) * 35)) | 0,
        Math.min(255, Math.max(0, bb + (v - streak) * 20)) | 0,
      );
    }
  }
...
function drawMountain(data: Uint8ClampedArray, height: number, seed: number): void {
  ...
  if (height === 0) {
    ...
    for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 0, lr, lg, lb, 180);
    for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 1, lr, lg, lb, 80);
    return;
  }
  ...
  for (let y = 0; y < TILE_SIZE; y++) {
    for (let x = 0; x < TILE_SIZE; x++) {
      const v = rng() * 0.14 - 0.07;
      setPixel(data, x, y,
        Math.min(255, Math.max(0, br + v * 50)) | 0,
        Math.min(255, Math.max(0, bg + v * 50)) | 0,
        Math.min(255, Math.max(0, bb + v * 55)) | 0,
      );
    }
  }
  ...
  for (let c = 0; c < crackCount; c++) {
    ...
    for (let d = 0; d < crackLen; d++) {
      ...
      if (cx > 0) setPixel(data, cx - 1, cy, lr, lg, lb, 60);
      cy++;
  ...
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 0, lr, lg, lb, 220);
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 1, lr, lg, lb, 110);
  for (let y = 2; y < TILE_SIZE; y++) setPixel(data, 0, y, lr, lg, lb, 160);
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, TILE_SIZE - 1, dr, dg, db, 220);
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, TILE_SIZE - 2, dr, dg, db, 100);
  for (let y = 0; y < TILE_SIZE - 2; y++) setPixel(data, TILE_SIZE - 1, y, dr, dg, db, 160);
  ...
  if (height === 3) {
    ...
    for (let y = 0; y < 11; y++) {
      for (let x = 0; x < TILE_SIZE; x++) {
        ...
        if (a > 0) setPixel(data, x, y, 215, 228, 240, a);
      }
    }
    // Eiskristall-Pixel
    for (let i = 0; i < 8; i++) {
      ...
      if (sx > 0) setPixel(data, sx - 1, sy, 230, 238, 248, 180);
      if (sx < 31) setPixel(data, sx + 1, sy, 230, 238, 248, 180);
      if (sy > 0) setPixel(data, sx, sy - 1, 230, 238, 248, 150);
    }
  }
}

function drawDesert(data: Uint8ClampedArray, height: number, seed: number): void {
  ...
  for (let y = 0; y < TILE_SIZE; y++) {
    for (let x = 0; x < TILE_SIZE; x++) {
      ...
      setPixel(data, x, y,
        Math.min(255, Math.max(0, br + v * 30)) | 0,
        Math.min(255, Math.max(0, bg + v * 25)) | 0,
        Math.min(255, Math.max(0, bb + v * 15)) | 0,
      );
    }
  }
  ...
  if (height >= 2) {
    for (let y = 8; y < TILE_SIZE; y += 5) {
      for (let x = 1; x < TILE_SIZE - 1; x++) {
        if (x % 3 !== 0) setPixel(data, x, y, lr, lg, lb, 70);
      }
    }
  }
  ...
  // Highlight-Kante
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 0, lr, lg, lb, 180);
  for (let y = 1; y < TILE_SIZE; y++) setPixel(data, 0, y, lr, lg, lb, 110);
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, TILE_SIZE - 1, dr, dg, db, 130);
  for (let y = 0; y < TILE_SIZE - 1; y++) setPixel(data, TILE_SIZE - 1, y, dr, dg, db, 100);
}

function drawDungeon(data: Uint8ClampedArray, height: number, seed: number): void {
  ...
  if (height === 0) {
    ...
    for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 0, dr, dg, db, 120);
    return;
  }
  ...
  for (let y = 0; y < TILE_SIZE; y++) {
    for (let x = 0; x < TILE_SIZE; x++) {
      ...
      const v = rng() * 0.04;
      setPixel(data, x, y,
        Math.min(255, br + tint + v * 15) | 0,
        Math.min(255, bg + tint + v * 15) | 0,
        Math.min(255, bb + tint + v * 20) | 0,
      );
    }
  }
  ...
  // Kanten-Highlights
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 0, lr, lg, lb, 200);
  for (let x = 0; x < TILE_SIZE; x++) setPixel(data, x, 1, lr, lg, lb, 90);
  for (let y = 2; y < TILE_SIZE; y++) setPixel(data, 0, y, lr, lg, lb, 130);
}
...
type DrawFn = (data: Uint8ClampedArray, height: number, seed: number) => void;

const DRAW_FUNCTIONS: Record<string, DrawFn> = {
  forest:   drawForest,
  swamp:    drawSwamp,
...
const BIOME_ORDER = ["forest", "swamp", "highland", "mountain", "desert", "dungeon"] as const;
 ...
 */
export function generateTileset(scene: any): void {
